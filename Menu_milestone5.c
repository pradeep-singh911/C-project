...
int  seats_available[Buses] = {50,50,50};
int bus_fare[Buses]= {950,1200,1700};
int seats[Buses][Seats] = {0};
   ...
   for (int i = 0; i < Buses; i++)
       {
        printf("\nBus No: Bus Name  :  Pickup       Destination    Fare   \n");
        printf("%d.     %s =>  %s:**To** %s:  /-%d\n",
         i + 1, bus_names[i],bus_pick[i],bus_dest[i],bus_fare[i]);
                }
}
        ...
        void interface()
        {
                     ...
                     while (1)
          {
            ...
            show_buses();
            printf("Enter your Choice (1-%d): ", Buses);
            scanf("%d", &bus_no);
            ...
            printf("Enter number of seats you want To book: 1-%d: ",Seats);
            scanf("%d", &multi_seat);
              
            if (multi_seat < 1 || multi_seat > Seats)
            {
            printf(" Invalid seat number!\n");
            return;
            }
            printf("\nBooking seats for passenger: %s\n", passenger);
            
            for (int j = 0; j < multi_seat;j++)
         {
            printf("\nEnter seat number to book (1-%d): ", Seats);
            scanf("%d", &seat_no);
